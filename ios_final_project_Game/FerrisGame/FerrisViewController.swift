...
class FerrisViewController: BaseViewController {
    ...
    func isCrash(point:CGPoint) -> Bool {
        for tmpPonit in self.strokePoints {
            let prePoint = tmpPonit.cgPointValue
            let dis = sqrt(pow(prePoint.x - point.x, 2) + pow(prePoint.y - point.y, 2))
            if(dis <= 2 * kTopRoundRadius + 1){
                self.centralAxisLayer.pauseAnimation()  //  停止动画
                return true
            }
        }
    ...
    lazy var centralAxisLayer:CAShapeLayer = {
        ...
        tmpLayer.lineWidth = 1.0
        tmpLayer.strokeColor = UIColor.darkGray.cgColor
        tmpLayer.fillColor = UIColor.darkGray.cgColor
        return tmpLayer
    }()
