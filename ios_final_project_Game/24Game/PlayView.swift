...
class PlayView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.num1 = self.createLabel(name: "1")
        self.num1.tag = 1
        self.num2 = self.createLabel(name: "2")
        self.num2.tag = 2
        self.num3 = self.createLabel(name: "3")
        self.num3.tag = 3
        self.num4 = self.createLabel(name: "4")
        self.num4.tag = 4
        
        self.dic = ["one":"0","two":"0","three":"0","four":"0"]
        
        self.opAdd = self.createLabel(name: "+")
        self.opSub = self.createLabel(name: "-")
        self.opMul = self.createLabel(name: "x")
        self.opDev = self.createLabel(name: "/")
    ...
    func check(label:UILabel) -> Void {
        ...
        if(text == "+" || text == "-" || text == "x" || text == "/"){
            ...
            if(labelFrame.intersects(ex1Frame)){
                self.ex1.text = text
                label.isHidden = true
                self.dealNumber(str: "one", tag: label.tag)
            }else if(labelFrame.intersects(ex3Frame)){
                self.ex3.text = text
                label.isHidden = true
                self.dealNumber(str: "two", tag: label.tag)
            }else if(labelFrame.intersects(ex5Frame)){
                self.ex5.text = text
                label.isHidden = true
                self.dealNumber(str: "three", tag: label.tag)
            }else if(labelFrame.intersects(ex7Frame)){
                self.ex7.text = text
                label.isHidden = true
                self.dealNumber(str: "four", tag: label.tag)
            }else{
                label.center = origal
            }
        }
    ...
    // 如果数字被拖进输入框，则隐藏
    func dealNumber(str:String,tag:Int) -> Void {
